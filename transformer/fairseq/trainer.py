...
from fairseq.optim.lr_scheduler.inverse_square_root_schedule import net_esd_estimator, get_layer_temps
...
class Trainer(object):
    ...
    def __init__(self, args, task, model, criterion, dummy_batch=None, oom_batch=None, ori_model=None):
        self.args = args
        print("---------------------> Trainer: save_dir", self.args.save_dir)
        self.task = task
    ...
    def optimizer(self):
        if self._optimizer is None:
            print(f"--------------------> property: self._build_optimizer()")
    ...
    def lr_scheduler(self):
        if self._lr_scheduler is None:
            print(f"--------------------> property _lr_scheduler: self._build_optimizer()")
            self._build_optimizer()  # this will initialize self._lr_scheduler
        return self._lr_scheduler

    def _build_optimizer(self):
        print("--------------------> _build_optimizer")
        
        
        if self.args.tb == 'True':
            metrics = net_esd_estimator(self._model, 
            ...
            # 0 learning rate for place holder
            scheduled_lr = get_layer_temps(self.args, 
                                            temp_balance=self.args.temp_balance_lr, 
                                            n_alphas=metric_scores, 
                                            epoch_val=0, metrics=metrics)
            if self.args.temp_balance_lr == 'tb_layer_groups':
                scheduled_lr = scheduled_lr.fill(0)
            ...
            for name, module in self._model.named_modules():
                if name in layer_name_to_tune:
                    ...
                    norm1_count += 1
                    #proj_name = name.replace('self_attn_layer_norm', 'self_attn.out_proj')
                    #print(f'Norm1 {name} {proj_name}')
                    params_to_tune_ids += list(map(id, module.parameters()))
                    scheduled_lr = \
                        layer_stats[layer_stats['longname'] == name.replace('self_attn_layer_norm', 'self_attn.out_proj')]['scheduled_lr'].item()
                    params.append({'params': module.parameters(), 'lr': scheduled_lr})
                    ...
                    norm2_count += 1
                    #proj_name = name.replace('final_layer_norm', 'fc2')
                    #print(f'Norm2 {name} {proj_name}')
                    params_to_tune_ids += list(map(id, module.parameters()))
                    scheduled_lr = \
                        layer_stats[layer_stats['longname'] == name.replace('final_layer_norm', 'fc2')]['scheduled_lr'].item()
                    params.append({'params': module.parameters(), 'lr': scheduled_lr})
                    ...
                    norm3_count += 1
                    #proj_name = name.replace('encoder_attn_layer_norm', 'encoder_attn.out_proj')
                    #print(f'Norm3 {name} {proj_name}')
                    params_to_tune_ids += list(map(id, module.parameters()))
                    scheduled_lr = \
                        layer_stats[layer_stats['longname'] == name.replace('encoder_attn_layer_norm', 'encoder_attn.out_proj')]['scheduled_lr'].item()
                    
                    params.append({'params': module.parameters(), 'lr': scheduled_lr})
            
            #print("Linear:", linear_count, "Norm1:", norm1_count, "Norm2:", norm2_count, "Norm3:", norm3_count)
            untuned_params = filter(lambda p: id(p) not in params_to_tune_ids, self._model.parameters())
            
            params.append({'params': untuned_params, 'lr': 0}) 
            #params.append({'params': self.criterion.parameters(), 'lr': 0}) 
            #print("--------------------> self.criterion.parameters()", filter(lambda p: p.requires_grad, self.criterion.parameters()))
        elif self.args.acc_v == 'True' or self.args.acc_mid == 'True':
            params = []
            params_to_tune_ids = []
            metrics = net_esd_estimator(self._model, 
                    EVALS_THRESH = 0.00001,
            ...
            for name, _ in self._model.named_modules():
                if 'v_proj' in name and 'decoder' in name and 'encoder' in name:
                    v_layers += 1
            ...
            for name, module in self._model.named_modules():
                if 'v_proj' in name and 'decoder' in name and 'encoder' in name:
                    ...
                    v_lr = start_lr * (self.args.lr_min_ratio + self.args.lr_slope * (v_layers - v_layer_count) / v_layers)
                    params.append({'params': module.parameters(), 'lr': v_lr})
                    params_to_tune_ids += list(map(id, module.parameters()))
                    v_layer_count += 1
                elif name in layer_name_to_tune:
                    params.append({'params': module.parameters(), 'lr': 0})
                    params_to_tune_ids += list(map(id, module.parameters()))
        ...
        if self.args.fp16:
            ...
            print("#############Initialize this optimizer!!!!!")
        ...
        print("We should initialize the learning rate scheduler immediately")
        self._lr_scheduler = lr_scheduler.build_lr_scheduler(self.args, self.optimizer, self._model, self._ori_model)
    ...
    def load_checkpoint(
        ...
        print("--------------------> Trainer.py: load_checkpoint")
        try:
        ...
        if last_optim_state is not None and not reset_optimizer:
            ...
            print("--------------------> Function: load_checkpoint, self._build_optimizer()")
            self._build_optimizer()
    ...
    def train_step(self, samples, dummy_batch=False, raise_oom=False):
        ...
        for i, sample in enumerate(samples):
            ...
            try:
                with maybe_no_sync():
                    ...
                    # We add gradsH for AdamHess
                    loss, sample_size, logging_output, gradsH = self.task.train_step(
        ...
        try:
            ...
            self.optimizer.step(gradsH=gradsH) # add gradsH for Hessian Computation!
            self.set_num_updates(self.get_num_updates() + 1)
            #print("num_updates:", self._num_updates)
    ...
    def set_num_updates(self, num_updates):
        ...
        #print(f"Fairseq Trainer.py: Set the number of parameters update here, {num_updates}")
