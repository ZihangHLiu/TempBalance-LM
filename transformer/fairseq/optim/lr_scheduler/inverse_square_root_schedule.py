...
def calc_ranked_metrics(metric, ori, scale):
    res = np.zeros(len(metric))
    idxs = np.argsort(metric)
    rev_idxs = idxs[::-1]
    for idx, val in enumerate(metric):
        rank = np.where(rev_idxs==idx)[0][0]
        res[idx] = ori * (1 + (scale - 1) * rank / len(metric))
    return res

def get_layer_temps(args, temp_balance, n_alphas, epoch_val, metrics=None):
...
def net_esd_estimator(
    ...
    for name, m in net.named_modules():
        if isinstance(m, nn.Conv2d) or isinstance(m, nn.Linear):
            ...
            results['xmin'].append(xmin_val.detach().cpu())
...
class InverseSquareRootSchedule(FairseqLRScheduler):
    ...
    def step_update(self, num_updates):
        ...
        #print(f"################{self.lr}")
        if num_updates % self.args.tb_interval == 0:
        ...
        if self.args.tb == 'True' \
            ...
            print(f'----------> number of V layers: {v_layers + 1}')
            block_lr = []
            l = np.array([i for i in range(v_layers + 1)])
            l = abs(l - max(l) // 2)
            l = len(l) / 2 - l
            max_elem = max(l)
            esd_lr = []
            for name, _ in self.model.named_modules():
                ...
                if name in layer_name_to_tune and 'decoder' in name:
                    assert match is not None
                    layer_idx = int(match.group(2))
                    v_lr = self.lr * (self.args.lr_min_ratio + l[layer_idx] * self.args.lr_slope / max_elem)
                    all_params_lr.append(v_lr)
                    v_layer_count += 1
                    c = c + 1
                    esd_lr.append(v_lr)
        ...
        elif self.args.acc_block == 'True' \
                and (self.args.tbr_after_warm == 'False' \
                    or (num_updates >= self.args.warmup_updates)):
                    
            print("Scheduling with block acceleration")
            ...
            for name, _ in self.model.named_modules():
                match = re.search(r'(\w+)\.layers\.(\d+)\.', name)
                if name in layer_name_to_tune and 'decoder' in name and match is not None and int(match.group(2)) == 2:
                    assert match is not None and self.args.lr_scale > 0 and self.args.lr_scale != 1
                    layer_idx = int(match.group(2))
                    # v_lr = self.lr * (1 + (self.args.lr_scale - 1) * (v_layers - layer_idx) / v_layers)
                    v_lr = self.lr * self.args.lr_scale
                    all_params_lr.append(v_lr)
                    v_layer_count += 1
                    c = c + 1
                    esd_lr.append(v_lr)
            ...
            optim_layer_num = 0
            for index, param_group in enumerate(self.optimizer._optimizer.param_groups):
                if index < len(all_params_lr):
                    # param_group['lr'] = all_params_lr[index]
                    param_group['lr'] = esd_lr[index]
                else:
                    param_group['lr'] = self.lr
