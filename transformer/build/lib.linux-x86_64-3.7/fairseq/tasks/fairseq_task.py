...
class FairseqTask(object):
    ...
    def train_step_hessian(self, samples, model, criterion, optimizer, ignore_grad=False, \
                                 v=None):
        """
        ...
        model.zero_grad()
        param_, gradsH = get_params_grad(model)
        THv = [ torch.zeros(p.size()).cuda() for p in param_ ]
        total_size = 0
        for sample in samples:
            ...
            param_, gradsH = get_params_grad(model) # this is used for AdamHess
        ...
        THv = [THv1 / float(total_size) for THv1 in THv]
        eigenvalue = group_product(THv, v).cpu().item()

        return loss, sample_size, logging_output, gradsH, eigenvalue, THv


    def train_step(self, sample, model, criterion, optimizer, ignore_grad=False, ignore_optimizer=False):
        """
        Do forward and backward, and return the loss as computed by *criterion*
        for the given *model* and *sample*.
        ...
        Returns:
            tuple:
                ...
                - logging outputs to display while training
        """
        #print(f"File Fairseq task train 1 step")
        model.train()
        loss, sample_size, logging_output = criterion(model, sample)
        if ignore_grad:
            loss *= 0
        if not ignore_optimizer:
            ...
            gradsH = None
        return loss, sample_size, logging_output, gradsH
